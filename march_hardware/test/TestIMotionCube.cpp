...
protected:
  ...
  march4cpp::EncoderIncremental encoderIncremental;
  march4cpp::ActuationMode actuationMode;
  void SetUp() override
  {
    encoder = march4cpp::Encoder();
    encoderIncremental = march4cpp::EncoderIncremental();
  }
};
...
{
  march4cpp::IMotionCube imc = march4cpp::IMotionCube(1, encoder, encoderIncremental);
  ASSERT_EQ(1, imc.getSlaveIndex());
}

TEST_F(IMotionCubeDeathTest, SlaveIndexZero)
{
  ASSERT_DEATH(march4cpp::IMotionCube(0, encoder, encoderIncremental), "Slave configuration error: slaveindex 0 can not be smaller than "
                                                   "1.");
}

TEST_F(IMotionCubeTest, SlaveIndexMinusOne)
{
  ASSERT_DEATH(march4cpp::IMotionCube(-1, encoder, encoderIncremental), "Slave configuration error: slaveindex -1 can not be smaller than "
                                                    "1.");
}
